...
void ADC_InjectedInit(ADC_TypeDef* ADCx, ADC_InjectedInitTypeDef* ADC_InjectedInitStruct)
{
  ...
  */
  tmpreg1 = (uint32_t) ((ADC_InjectedInitStruct->ADC_NbrOfInjecChannel - (uint8_t)1) |
                         ...
                         (uint32_t)((ADC_InjectedInitStruct->ADC_InjecSequence1) << 9) |
                         (uint32_t)((ADC_InjectedInitStruct->ADC_InjecSequence2) << 15) |
                         (uint32_t)((ADC_InjectedInitStruct->ADC_InjecSequence3) << 21) |
                         (uint32_t)((ADC_InjectedInitStruct->ADC_InjecSequence4) << 27));
  /* Write to ADCx SQR1 */
  ADCx->JSQR = tmpreg1;  
}
...
{
  ADC_InjectedInitStruct->ADC_ExternalTrigInjecConvEvent = 0;    
  ADC_InjectedInitStruct->ADC_ExternalTrigInjecEventEdge = ADC_ExternalTrigInjecEventEdge_None;     
  ADC_InjectedInitStruct->ADC_NbrOfInjecChannel = 0;                                                             
  ADC_InjectedInitStruct->ADC_InjecSequence1 = 0; 
  ADC_InjectedInitStruct->ADC_InjecSequence2 = 0;
  ADC_InjectedInitStruct->ADC_InjecSequence3 = 0;
  ADC_InjectedInitStruct->ADC_InjecSequence4 = 0; 
}
...
uint32_t ADC_CH0GetCalibrationValue(ADC_TypeDef* ADCx)
{
  ...
  return (uint32_t)((ADCx->CALFACT & ADC_CALFACT_CH0_Msk) >> ADC_CALFACT_CH0_Pos);
}
...
void ADC_CH0SetCalibrationValue(ADC_TypeDef* ADCx, uint32_t ADC_Calibration)
{
  ...
  /* Get the ADC calibration register */
  tmpreg1 = ADCx->CALFACT;
  
  /* Clear CALFACT bit */
  tmpreg1 &= ~ADC_CALFACT_CH0_Msk;
  
  /* Set the ADC calibration register value */
  tmpreg1 |= (ADC_Calibration << ADC_CALFACT_CH0_Pos);

  /* Write the ADC calibration register */
  ADCx->CALFACT = tmpreg1;
}
...
uint32_t ADC_CH1GetCalibrationValue(ADC_TypeDef* ADCx)
{
  ...
  /* Return the selected ADC calibration value */
  return (uint32_t)((ADCx->CALFACT & ADC_CALFACT_CH1_Msk) >> ADC_CALFACT_CH1_Pos);
}
...
void ADC_CH1SetCalibrationValue(ADC_TypeDef* ADCx, uint32_t ADC_Calibration)
{
  ...
  /* Clear CALFACT bit */
  tmpreg1 &= ~ADC_CALFACT_CH1_Msk;
  
  /* Set the ADC calibration register value */
  tmpreg1 |= (ADC_Calibration << ADC_CALFACT_CH1_Pos);
...
void ADC_SelectQueueOfContextMode(ADC_TypeDef* ADCx, FunctionalState NewState)
{
  ...
  {
    /* Set the JQM bit */
    ADCx->CFGR1 |= (uint32_t)(ADC_CFGR1_JQM );
  }
  else
  {
    /* Reset the JQM bit */
...
void ADC_AnalogWatchdogCmd(ADC_TypeDef* ADCx, uint32_t ADC_AnalogWatchdog)
{
  ...
  tmpreg &= ~(uint32_t)(ADC_CFGR1_AWD1SGL|ADC_CFGR1_AWD1EN|ADC_CFGR1_JAWD1EN);
...
void ADC_AnalogWatchdog1SingleChannelConfig(ADC_TypeDef* ADCx, uint8_t ADC_Channel)
{
  ...
  tmpreg &= ~(uint32_t)ADC_CFGR1_AWD1CH;
...
/**
  ...
  *     @arg ADC_SampleTime_6Cycles5: Sample time equal to 6.5 cycles
  *     @arg ADC_SampleTime_12Cycles5: Sample time equal to 12.5 cycles
  *     @arg ADC_SampleTime_24Cycles5: Sample time equal to 24.5 cycles	
  *     @arg ADC_SampleTime_47Cycles5: Sample time equal to 47.5 cycles	
  *     @arg ADC_SampleTime_92Cycles5: Sample time equal to 92.5 cycles	
  *     @arg ADC_SampleTime_247Cycles5: Sample time equal to 247.5 cycles	
  *     @arg ADC_SampleTime_640Cycles5: Sample time equal to 640.5 cycles	
  * @retval None
  */
void ADC_RegularChannelConfig(ADC_TypeDef* ADCx, uint8_t ADC_Channel, uint8_t Rank, uint8_t ADC_SampleTime)
{
  ...
  else 
  {
    ...
    tmpreg1 = ADCx->SQR4;
    /* Calculate the mask to clear */
    tmpreg2 = ADC_SQR4_SQ15 << (6 * (Rank - 15));
    /* Clear the old SQx bits for the selected rank */
    tmpreg1 &= ~tmpreg2;
    /* Calculate the mask to set */
  ...
  if (ADC_Channel < ADC_Channel_10)
  {
    ...
    /* Set the sample bits for the selected channel */
    tmpreg1 |= tmpreg2;
    /* Store the new register value */
    ADCx->SMPR1 = tmpreg1;
  }
  else /* if ADC_Channel_10 ... ADC_Channel_15 is selected */
  {
    /* Get the old register value */
    tmpreg1 = ADCx->SMPR2;
    /* Calculate the mask to clear */
...
void ADC_ExternalTriggerConfig(ADC_TypeDef* ADCx, uint16_t ADC_ExternalTrigConvEvent, uint16_t ADC_ExternalTrigEventEdge)
{
  ...
  ADCx->CFGR1 &= ~(ADC_CFGR1_EXTEN | ADC_CFGR1_EXTSEL);
  ADCx->CFGR1 |= (uint32_t)(ADC_ExternalTrigEventEdge | ADC_ExternalTrigConvEvent);
}
...
void ADC_DiscModeChannelCountConfig(ADC_TypeDef* ADCx, uint8_t Number)
{
  ...
  tmpreg1 &= ~(uint32_t)(ADC_CFGR1_DISCNUM);
...
void ADC_JDMACmd(ADC_TypeDef* ADCx, FunctionalState NewState)
{
  ...
  {
    /* Enable the selected ADC DMA request */
    ADCx->CFGR1 |= ADC_CFGR1_JDMAEN;
  }
  else
  {
    /* Disable the selected ADC DMA request */
    ADCx->CFGR1 &= ~(uint32_t)ADC_CFGR1_JDMAEN;
  }
}
...
void ADC_InjectedChannelSampleTimeConfig(ADC_TypeDef* ADCx, uint8_t ADC_InjectedChannel, uint8_t ADC_SampleTime)
{
  ...
  /* Channel sampling configuration */  
  /* ADC_Channel include in ADC_InjectedChannel_[0..9] */
  if (ADC_InjectedChannel < ADC_Channel_10)
  {
    /* Get the old register value */
    tmpreg1 = ADCx->SMPR1;
    /* Calculate the mask to clear */
    tmpreg2 = ADC_SMPR1_SMP0 << (3 * (ADC_InjectedChannel));
    /* Clear the old channel sample time */
    tmpreg1 &= ~tmpreg2;
    /* Calculate the mask to set */
    tmpreg2 = (uint32_t)ADC_SampleTime << (3 * (ADC_InjectedChannel));
    /* Set the sample bits for the selected channel */
    tmpreg1 |= tmpreg2;
    /* Store the new register value */
    ADCx->SMPR1 = tmpreg1;
  }
  else /* if ADC_InjectedChannel_10 ... ADC_InjectedChannel_15 is selected */
  {
    /* Get the old register value */
    tmpreg1 = ADCx->SMPR2;
    /* Calculate the mask to clear */
    tmpreg2 = ADC_SMPR2_SMP10 << (3 * (ADC_InjectedChannel - 10));
    /* Clear the old channel sample time */
    tmpreg1 &= ~tmpreg2;
    /* Calculate the mask to set */
    tmpreg2 = (uint32_t)ADC_SampleTime << (3 * (ADC_InjectedChannel - 10));
    /* Set the sample bits for the selected channel */
    tmpreg1 |= tmpreg2;
    /* Store the new register value */
    ADCx->SMPR2 = tmpreg1;
...
{
	ADC1->CCR &= ~ADC_CCR_REFSEL;
	ADC1->CCR |= VREFSelection;
