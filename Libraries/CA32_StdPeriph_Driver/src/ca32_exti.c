    ...
    *(__IO uint32_t *) tmp |= EXTI_InitStruct->EXTI_Line;
    ...
    else
    {
      ...
      *(__IO uint32_t *) tmp |= EXTI_InitStruct->EXTI_Line;
		...
		EXTICFG_StopWackupModeConfig(EXTI_Wakeup_Ex);
  }
  else
  {
    tmp += EXTI_InitStruct->EXTI_Mode;
...
/**
  ...
  *         will be generated.
  *          This parameter can be any combination of EXTI_Linex where x can be (0..18).
  * @retval None
  */
...
/**
  ...
  *          This parameter can be EXTI_Linex where x can be (0..18).
...
{
	EXTICFG->EXTI_CFGR &= ~EXTICFG_WKUP_SEL;
	EXTICFG->EXTI_CFGR |= WKUP_Mode<<EXTICFG_WKUP_SEL_Pos;
}
