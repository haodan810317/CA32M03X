...
void GPIO_Init(GPIO_TypeDef* GPIOx, GPIO_InitTypeDef* GPIO_InitStruct)
{
  ...
  for (pinpos = 0x00; pinpos < 0x10; pinpos++)
  {
    ...
    if (currentpin == pos)
    {
      ...
      {
        /* ODriver mode configuration */
				GPIOx->ODRIVER  &= ~(GPIO_ODRIVER_0 << (pinpos * 2));
        GPIOx->ODRIVER |= (((uint32_t)GPIO_InitStruct->GPIO_ODrive) << (pinpos * 2));

        /* Check Output mode parameters */
        assert_param(IS_GPIO_OTYPE(GPIO_InitStruct->GPIO_OType));

        /* Output mode configuration */
        GPIOx->OTYPER &= ~((GPIO_OTYPER_OD_0 | GPIO_OTYPER_OS_0) << ((uint16_t)pinpos));
        GPIOx->OTYPER |= (uint32_t)(((uint32_t)GPIO_InitStruct->GPIO_OType) << ((uint16_t)pinpos));
      }
      ...
      GPIOx->ITYPER &= ~(GPIO_ITYPER_0 << ((uint16_t)pinpos));
      GPIOx->ITYPER |= (uint16_t)(((uint16_t)GPIO_InitStruct->GPIO_IType) << ((uint16_t)pinpos));
    }
  }
}
...
void GPIO_StructInit(GPIO_InitTypeDef* GPIO_InitStruct)
{
  ...
  GPIO_InitStruct->GPIO_ODrive = GPIO_ODrive_VeryHigh;
  GPIO_InitStruct->GPIO_OType = GPIO_OType_PP;
  GPIO_InitStruct->GPIO_PuPd = GPIO_PuPd_NOPULL;
  GPIO_InitStruct->GPIO_IType = GPIO_IType_Schmitt;
}
...
/**
  ...
  * @note   This parameter can be GPIO_Pin_x where x can be: 0 to 15
...
/**
  * @brief  Toggles the selected data port bits.
  * @param  GPIOx: where x can be (A or B) to select the GPIO peripheral.
  * @param  GPIO_Pin: specifies the port bits to be written.
  * @note   This parameter can be GPIO_Pin_x where x can be: 0 to 15
  * @retval None
...
/**
  * @brief  Toggles data to the specified GPIO pin.
  * @param  GPIOx: where x can be (A or B) to select the GPIO peripheral.
  * @param  GPIO_Pin: specifies the port bit to be written.
  * @note   This parameter can be GPIO_Pin_x where x can be: 0 to 15
  * @retval None
...
/**
  ...
  *          This parameter can be GPIO_PinSourcex where x can be (0..15) for GPIOA or GPIOB
  * @param  GPIO_AF: selects the pin to used as Alternate function.
  *          This parameter can be one of the following value: GPIO_AF_x where x can be (0..15)
